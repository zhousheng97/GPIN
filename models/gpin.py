...
from pythia.modules.PAM import Q, QT, QTV
from pythia.modules.Image_Text_Matching import ITM
from pythia.modules.Select_Topk import Select_Topk
from pythia.modules.Graph_Module import Graph_Module



@registry.register_model("gpin")
class GPIN(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.PAM_config = BertConfig(**self.config.PAM)  # Progressive Attention Module
        self.ITM_config = self.config.ITM  # Image Text Matching
        self.TopK_config = self.config.Select_TopK  # Select TopK Regions
        self.Graph_config = BertConfig(**self.config.MRG)  # Multimodal Reasoning Graph
    ...
    def _build_model(self):
        ...
        self.ITM = ITM(self.ITM_config)
        self.Select_Topk = Select_Topk(self.TopK_config)
        self.Graph_Module = Graph_Module(self.Graph_config)
    ...
    def _forward_pam_graph(self, sample_list, fwd_results):
        ...
        self.ITM(fwd_results, sample_list)
        self.Select_Topk(fwd_results, sample_list)
        self.Graph_Module(sample_list, fwd_results)
    ...
    def _forward_mmt(self, sample_list, fwd_results):
        ...
        mmt_results = self.mmt(
            ...
            graph_emb=fwd_results['graph_embedding'],
            graph_emb_mask=fwd_results['graph_mask'],
...
class MMT(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        encoder_inputs = torch.cat(
            [txt_emb, obj_emb, ocr_emb, graph_emb, dec_emb],
            dim=1
        )
        attention_mask = torch.cat(
            [txt_mask, obj_mask, ocr_mask, graph_emb_mask, dec_mask],
            dim=1
        )
